...
const Products = ({ setCurrentPage }) => {
  ...
  const openEditModal = (product) => {
    ...
    setImagePreview(product.image ? `${process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000'}/images/products/${product.image}` : null);
    setFormError(null);
    setIsModalOpen(true);
  };
                  ...
                  <tr key={product.id}>
                    <td>
                      <div className="prod-img">
                        <img src={`${process.env.APP_URL}/images/products/${product.image}`} alt={product.name} />
                      </div>
                    </td>
